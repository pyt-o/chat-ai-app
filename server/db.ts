...
export async function createConversation(userId: number, title: string, mode: "general" | "code" | "narrator" = "general") {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  
  const result = await db.insert(conversations).values({ userId, title, mode });
